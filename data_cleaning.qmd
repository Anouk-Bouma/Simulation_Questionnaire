...
<!--# NOTE: Maybe split into a new script from here -->
...
n_invitations <- 695
n_participants <- 244
response_rate <- n_participants/n_invitations # = 0.3511



# NOTE: incorrect and probably can be removed
#Completion Rate
# completed <- data %>%
#   count(Finished)
# 
...
####
# Addition of new answer categories based on themes found in open answers (NOTE see codebook for details)
...
# Note that coding 'other' answers to 8 is done per newly added answer category, not per question to keep the code as easily interpretable as possible.

data <- data %>%
  mutate( # Keyword: Not Applicable
    Q16a_16_NEW = if_else(
      ID %in% c("id9", "id69"), 
      1, 
      NA_real_),
    # Q16a_15 is option 'other' -> changed to 8 (see codebook)
    Q16a_15 = if_else(
      ID %in% c("id9", "id69"), 
      8, 
      Q16a_15),
    # Q16a_16_NEW is placed after the last Q16 column
    .after = "Q16a_15_TEXT" 
    ) %>%
  mutate( # Keyword: Not Applicable
    Q18a_11_NEW = if_else(
      ID %in% c("id9", "id156", "id161", "id108", "id139"), 
      1, 
      NA_real_), 
    Q18a_10 = if_else(
      ID %in% c("id9", "id156", "id161", "id108", "id139"), 
  ...
  mutate( # Keyword: Low Occurrence
    Q16a_17_NEW = if_else(
      ID %in% c("id108", "id129", "id197", "id210"), 
      1, 
      NA_real_), 
    Q16a_15 = if_else(
      ID %in% c("id108", "id129", "id197", "id210"), 
  ...
  mutate( # Keyword: Alternative Check
    Q18a_13_NEW = if_else(
      ID %in% c("id60", "id71", "id137", "id178"), 
      1, 
      NA_real_), 
    Q18a_10 = if_else(
      ID %in% c("id60", "id71", "id137", "id178"), 
  ...
  mutate( # Keyword: Field
    Q20_9_NEW = if_else(
      ID %in% c("id27", "id36", "id105", "id165", "id191", "id204"), 
      1, 
      NA_real_), 
    Q20_8 = if_else(
      ID %in% c("id27", "id36", "id105", "id165", "id191", "id204"), 
  ...
  mutate( # Keyword: Experience
    Q20_10_NEW = if_else(
      ID %in% c("id35", "id36", "id43", "id135", "id140"), 
      1, 
      NA_real_), 
    Q20_8 = if_else(
      ID %in% c("id35", "id36", "id43", "id135", "id140"), 
  ...
  mutate( # Keyword: Not Available
    Q20_11_NEW = if_else(
      ID %in% c("id161", "id165", "id188", "id213"), 
      1, 
      NA_real_), 
    Q20_8 = if_else(
      ID %in% c("id161", "id165", "id188", "id213"), 
...
# Adding the same new categories to the b-versions of each respective question to aid data analysis later on.
data <- data %>%
  mutate(
    Q16b_16_NEW = NA,
    Q16b_17_NEW = NA,
...
# Renaming column names for 18b and adding a column to represent the first option for 18a (that was not part of 18b), to aid plotting.

data <- data %>%
  rename(
    Q18b_2 = Q18b_1, #Q18b_1 becomes Q18b_2
    Q18b_3 = Q18b_2,
    Q18b_4 = Q18b_3,
    Q18b_5 = Q18b_4,
    Q18b_6 = Q18b_5,
...
data <- data %>%
  ...
  mutate(Q16a_6 = if_else(
    ID %in% c("id96", "id159", "id164", "id178", "id199"), 
    1, 
    Q16a_6)) %>%
  mutate(Q16a_15 = if_else(
    ID %in% c("id96", "id159", "id164", "id178", "id199"), 
    8, 
    Q16a_15)) %>%
  # id9: change answers to questions about research guidelines
  mutate(Q19_5 = if_else(
    ID %in% c("id9"), 
    1, 
    Q19_5)) %>%
  mutate(Q19_6 = if_else(
    ID %in% c("id9"), 
    8, 
    Q19_6)) %>%
  mutate(Q20_8 = if_else(
    ID %in% c("id9"), 
    1, 
    Q20_8)) %>%
  mutate(Q20_8_TEXT = if_else(
    ID %in% c("id9"), 
    "Report all that is relevant to the research question", 
    Q20_8_TEXT)) %>%
  # id125: change answer to Q23_1 to TRUE
  mutate(Q23_1 = if_else(
    ID %in% c("id125"), 
    1, 
    Q23_1)) %>%
  mutate(Q23_5 = if_else(
    ID %in% c("id125"), 
    8, 
    Q23_5)) %>%
  # id14: change answer Q24_3 to TRUE
  mutate(Q24_3 = if_else(
    ID %in% c("id14"), 
    1, 
    Q24_3)) %>%
  ...
  mutate(Q16a_6 = if_else(
    ...
    1, 
    Q16a_6)) %>%
  # all answer options except for Q16a_6 set to 0 instead of NA:
  mutate(across(c(Q16a_1, Q16a_2, Q16a_3, Q16a_4, Q16a_5, 
...
data <- data %>%
  mutate(new_method = case_when(Q2 == 1 | 
                                Q2 == 2 | 
                                Q3 == 1 ~ 1, 
                                .default = 0), .after = "Q3")
```
