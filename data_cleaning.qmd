...
# Completion Rate
# NOTE: add completion rate (before any participant is removed)
```
...
# For participants where their answer option in the 'other' answer option is assigned to a new category, their score for the option 'other' is recoded as a 2 (see codebook). 
# Note that coding 'other' answers to 2 is done per category, not per question to keep the code as easily interpretable as possible.
data <- data %>%
  mutate(
    Q16a_16_NEW = if_else(ID %in% c("id9", "id69"), 1, NA_real_), # Keyword: Not Applicable
    Q16a_15 = if_else(ID %in% c("id9", "id69"), 2, Q16a_15), # Q16a_15 is option 'other' -> changed to 2 (see codebook)
...
data <- data %>%
  ...
  # Change answer Q25_12 to 3 for participant that did not indicate a reasons but gave a non-content related comment (see codebook)
  mutate(Q25_12 = if_else(ID %in% c("id201"), 3, Q25_12))
...
####
# Slider check: making slider questions (Q27_1, Q28_1, Q29_1) NA if they failed the slider check (Q26_1)
