...
[2] LC_CTYPE=Dutch_Netherlands.utf8    
[3] LC_MONETARY=Dutch_Netherlands.utf8 
[4] LC_NUMERIC=C                       
[5] LC_TIME=Dutch_Netherlands.utf8      

time zone: Europe/Amsterdam tzcode 
source: internal
```

To run the code, first run the following codeblock to attach the packages needed.
...
Here, some general functions objects and transformations are conducted that are necessary to run the other blocks of code. In principle, after running each of these general blocks, can be run separately from one another.
...
create_question_table <- function(data, metadata, sample_sizes) {
  ...
  # ---- calculate Percentage at the very end ----
  result <- result %>%
    mutate(
      Percentage = ifelse(
...
any_selective <- data %>%
  filter(conditions == 0 | methods == 0 | perfmeas == 0 | conditions == 2 | methods == 2 | perfmeas == 2)

any_selective_reported <- nrow(any_selective)/221  
```
...
### 4.1 Function non-splitted plots
...
p_rationale_missings <- plot_combined_questions(data,
                               ...
                               combine_ab = TRUE,
                               colors = "#D55E00",
                               title_plot = "The number of missing values/nonconvergent iterations wasn't reported for all methods and conditions (in the body of the paper) because of...",
                               y_max = 70,
                               wrap_width = 80)
...
### 4.3 Function Plots split by variable
...
plot_combined_questions_grouped <- function(
  ...
  plot_data <- counts %>%
    ...
    mutate(
      ...
      label = sprintf("%.1f%% (%d)", pct, freq),
      !!sym(split_var) := factor(!!sym(split_var), levels = rev(unique(!!sym(split_var)))), # ensures splitvar is a factor
      short_label = factor(
        short_label, 
        levels = rev(meta_sel %>% arrange(order) %>% distinct(short_label) %>% pull(short_label))
      )
    )
  ...
  ggplot(
    ...
    scale_fill_manual(
      ...
      breaks = rev(levels(plot_data[[split_var]])),
      name = ""
      ) +
    scale_x_discrete(
      labels = ~ stringr::str_wrap(.x, wrap_width)
...
for (i in 1:length(all_vars)) {
  ...
  current_title <- meta %>%
    filter(grepl(paste0("^", all_vars[i], "([a-z])?$"), question_group)) %>%
    slice(1) %>%
    pull(item)
  ...
  p <- plot_combined_questions_grouped(
    ...
    wrap_width = 100,
    title_plot = ifelse(all_vars[i] == "conditions", 
                        "Q5/Q6: Is at least one result reported for every condition that was part of the simulations?", 
                        ifelse(all_vars[i] == "methods", 
                               "Q8/Q9: Is at least one result reported for every method that was part of the simulations?",
                               ifelse(all_vars[i] == "perfmeas", 
                                      "Q11/Q12: Is at least one result reported for every performance measure that was part of the simulations?", paste0(all_vars[i], ": ", current_title))))
  )
  plot_list[[i]] <- p + theme(plot.margin = margin(1, 5, 1, 5, "cm"))
}

# Add plots to object to save
multipage_plot <- marrangeGrob(plot_list, nrow = 3, ncol = 1)
# Generate PDF with plots
ggsave("Plots_split_neutrality.pdf", multipage_plot, width = 21*1.5, height = 29.7*1.5, units = "cm")

########## Plots split by aim ##################

# Three participants that indicated 'none of the above applies' for Q2 are not used in plot
data_without_other <- data %>%
  filter(Q2 != 4)
...
p <- plot_combined_questions_grouped(
    data = data_without_other,
    metadata = meta,
    combine_ab = all_vars[i] %in% vars_to_plot_ab,
    combined_questions = all_vars[i],
    legend_labels = c("New method compared", "New method not compared", "Existing method(s)"),
...
plot_list_aim[[i]] <- p + theme(plot.margin = margin(1, 5, 1, 5, "cm"))
}

# Add plots to object to save
multipage_plot <- marrangeGrob(plot_list_aim, nrow = 2, ncol = 1)
# Generate PDF with plots
ggsave("Plots_split_aim.pdf", multipage_plot, width = 21*1.3, height = 29.7*1.3, units = "cm")
```
...
p_conditions <- plot_combined_questions_grouped(
    data = data,
    metadata = meta,
    combine_ab = FALSE,
    combined_questions = "conditions",
    split_var = "new_method",
    wrap_width = 100,
    title_plot = "Conditions")

p_methods <- plot_combined_questions_grouped(
    data = data,
    metadata = meta,
    combine_ab = FALSE,
    combined_questions = "methods",
    split_var = "new_method",
    wrap_width = 100,
    title_plot = "Methods")

p_perfmeas <- plot_combined_questions_grouped(
    data = data,
    metadata = meta,
    combine_ab = FALSE,
    combined_questions = "perfmeas",
    split_var = "new_method",
    wrap_width = 100,
    title_plot = "Performance Measures")

figure_4 <- (p_conditions + 
    p_methods + theme(
...
ggsave("Figure_4.png", plot = figure_4, width = 8.1*3, height = 4*3, units = "cm", dpi = 300)
