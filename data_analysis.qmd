...
-   Exclude everyone that did not fill out at least one content question = response rate after that
...
-   Change the answers of the dude that emailed

-   Make sure all numerical values are numbers (research experience) (if you are unsure make them missing, check with supervisors)
...
# Step 2: filter rows where not all selected columns are NA or -99
data_filtered <- data %>%
...
In this block I will extract all textual results that need to be evaluated to decide what to do with them.

```{r}

# Define the text columns you want to extract
text_cols <- c("Q7a_12_TEXT", "Q7b_12_TEXT", 
