...
# Metadata for combined plot
meta_combined <- read.xlsx("question_labels.xlsx", sheet = "codebook_combined")

# Sample sizes for combined plot
sample_sizes_combined <- meta_combined %>%
  distinct(question_group, question_code) %>%
  arrange(question_group) %>%
  group_by(question_group) %>%
  summarise(
    sample_col = {
      counts <- sapply(question_code, function(col) {
        sum(!is.na(df_plot[[col]]) & df_plot[[col]] != -99 & df_plot[[col]] != 999)
      })
      question_code[which(counts > 0)[1]]
    },
    n = {
...
pconditions <- plot_combined_questions(df_plot,
                               meta_combined,
                               sample_sizes = sample_sizes_combined,
