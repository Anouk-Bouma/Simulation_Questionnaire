...
plot_question <- function(
  ...
  if (n_distinct(meta_group$question_code) > 1) { 
    ...
    ggplot(aes(x = fct_reorder(answer_label, desc(order)))) +
      geom_bar(fill = "#f68060", alpha =0.6, width = 0.4) +
      coord_flip() +
      scale_y_reverse() +
      ...
      scale_x_discrete(labels = ~ stringr::str_wrap(.x, width = wrap_width),
                       position = "top") +
      labs(x = NULL, y = "Count", title = meta_group$item[1]) +
      theme_bw() #+
      # theme(
