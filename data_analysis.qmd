...
# Subsets of dataset (can also be done inside the plotting part itself)
data_newmethod <- data %>%
  filter(new_method ==1)

# Problem here is that the NA values are kept in the plot
ggplot(pivot_longer(data = data, 
                    cols = question_7a, # this gives a warning but does work. Maybe check for an alternative
                    names_to = "question",
                    values_to = "selected"), 
                    aes(question, fill = factor(selected))) +
  geom_bar()
...
data %>%
  ...
  ggplot(aes(x = question, fill = factor(selected))) +
  geom_bar()
