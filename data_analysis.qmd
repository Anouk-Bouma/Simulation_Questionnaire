...
for (i in 1:length(vars_to_plot_regular)) {
  ...
  # Assuming 'question_group' in metadata matches the strings in vars_to_plot_regular
  current_title <- meta %>%
    filter(question_group == vars_to_plot_regular[i]) %>%
    slice(1) %>%
    pull(item)
  ...
  p <- plot_combined_questions_grouped(
    ...
    split_var = "new_method",
    wrap_width = 50,
    title_plot = paste0(vars_to_plot_regular[i], ": ", current_title)  # Pass the dynamic title here
  )
...
########## Add plots with a/b versions here: ##############
