...
plot_question_percentage <- function(
  ...
  if (n_distinct(meta_group$question_code) > 1) { 
    ...
    # Total number of respondents (for denominator)
    total_n <- nrow(data)
    ...
    # ----- Plot for multi-select -----
    ggplot(plot_data, aes(x = fct_reorder(answer_label, desc(order)), y = pct)) +
      geom_col(fill = "#f68060", alpha = 0.6, width = 0.4) +
      coord_flip() +
    ...
    # Total responses for denominator
    total_n <- nrow(data %>% filter(!is.na(.data[[qcol]])))
...
for (i in 1:length(all_questions)) {
  ...
  print(plot_question_percentage(data, meta, all_questions[i]))
}
