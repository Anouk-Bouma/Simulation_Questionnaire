...
-   Check if Q14 and Q15 should also become a combined plot, then add new variable just as with conditions/methods/performance measures
...
for (i in 1:length(vars_to_plot_ab)) {
  plot_list[[i]] <- plot_combined_questions_grouped(
    data = data,
    metadata = meta,
    combine_ab = TRUE,
    combined_questions = vars_to_plot_ab[i],
    split_var = "new_method"
)
}

for (i in 1:length(vars_to_plot_regular)) {
  plot_list[[i]] <- plot_combined_questions_grouped(
    data = data,
    metadata = meta,
    combine_ab = FALSE,
    combined_questions = vars_to_plot_regular[i],
    split_var = "new_method"
)
}
...
plot_combined_questions_grouped(
    ...
    metadata = meta,
    combine_ab = FALSE,
    combined_questions = c("methods"),
    split_var = "new_method"
)
```

## Streamlined workflow Chi\^2 and Cramer's V with plots
...
plot_data <- get_plot_data(data = data, metadata = meta, combined_questions = vars_to_plot_regular, split_var = "new_method", combine_ab = FALSE)

calculate_stats <- function(plot_df, split_var) {
  # Create a wide contingency table: Rows = Answer labels, Cols = Group (0/1)
  test_table <- plot_df %>%
    mutate(id_col = paste0(combined, " ", short_label)) %>%
    select(id_col, all_of(split_var), freq) %>%
    pivot_wider(
      names_from = all_of(split_var), 
      values_from = freq) %>%
    column_to_rownames("id_col") %>%
    as.matrix()
  
  # Remove rows/cols with only zeros to avoid errors
  #test_table <- test_table[rowSums(test_table) > 0, colSums(test_table) > 0]
...
stats_data <- calculate_stats(plot_df = plot_data, split_var = "new_method")
```

```{r}


# We wrap your current logic into a 'Master' processing list
results_list <- map(vars_to_plot_regular, function(q_name) {
  
  # A. Use your existing logic to get the data (modify your function to return the data if needed)
  # For now, let's assume we extract the 'plot_data' part
  p_data <- get_plot_data(data, meta, q_name, "new_method", combine_ab = FALSE)
  
  # B. Run the stats
  stats_output <- calculate_stats(p_data, "new_method")
  
  # C. Generate the plot (your existing function)
  p_plot <- plot_combined_questions_grouped(data, meta, q_name, split_var = "new_method", combine_ab = FALSE)
  
  # D. Store everything in a clean list
  list(
    question = q_name,
    stats = stats_output,
    plot = p_plot,
...
# Create a summary table for your next calculations
summary_table <- map_df(results_list, function(item) {
  tibble(
    question = item$question,
    p_value = item$stats$p_value,
    cramers_v = item$stats$cramers_v,
    significant = item$stats$p_value < 0.05
  )
})
